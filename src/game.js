...
(() => {
  ...
  // 0段の夜（目付 k2s7・R-2 → 09-01 に宵あらわし式へ揃えた）: **一度も記録が無い人の0段だけ送らない**
  // ＝番付に「0段」の行を作らない。記録を持つ人の0段は送ってよい——番付は自己ベストしか持たず
  // 改善時しか書かれないので記録は下がらず、順位の行が「出たり出なかったり」しない
  // （宵あらわし 08-31 実機指摘）。`saveData.best` は夜の終わりに今夜の段を取り込む前に見る。
  const canSubmitScore = () => !practice && !autotest && !autocut && !autowobble && (floors > 0 || hadRecordBeforeTonight);

  let hadRecordBeforeTonight = false; // 夜の頭で「既に記録を持っているか」を控える（0段の送信判定に使う）
  function showGameOver() {
    clearRankUI(); // 前の夜の順位を残さない
    hadRecordBeforeTonight = saveData.best > 0;
    if (!practice) {
